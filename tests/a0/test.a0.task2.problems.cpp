...
Test test_a0_task2_problems_numerical("a0.task2.problems.numerical", []() {
    std::vector<std::vector<int>> target = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}; // 3 x 3 matrix
    ...
    int factor = 3;
    auto helper = [factor](int x, int y) -> bool { 
        return float(x) < (float(y) / float(factor)); 
    };

    int j = 0;
    for (auto& v : target) {  // auto& (int&) はtargetの中身を置き換え可能
        for (auto& i : v) {
...
Test test_a0_task2_problems_vector("a0.task2.problems.vector", []() {
    ...
    auto end = one_to_ten.end();
    --end;
    int last_element = *end;
